...
if (!cX.library) {
    cX.library = {
        ...
        clearIdUrl: cX.clearIdUrl || ('http' + (location.protocol == 'https:' ? 's://s' : '://') + 'comcluster.cxense.com/public/clearid'),
        ...
        clearBaseUrl: cX.clearBaseUrl || (location.protocol === 'https:' ? 'https://scdn.cxense.com/sclear.html' : 'http://cdn.cxense.com/clear.html'),
        ...
        cdn: {
            template: {
                direct: { http: 'http://cdn.cxpublic.com/', https: 'https://cdn.cxpublic.com/' },
                mapped: { http: 'http://cdn-templates.cxpublic.com/', https: 'https://cdn-templates.cxpublic.com/' }
            }
        },
        csdUrls: {
        ...
        emptyWidgetUrl: 'http' + (location.protocol == 'https:' ? 's://s' : '://') + 'cdn.cxense.com/empty.html',
        ...
        requireConsent: function(version) {
            ...
            if (version) this.m_consentVersion = version;
            this._initializeUnloadHandler();
        },
        isConsentRequired: function() { return !!this.m_usesConsent; },

        // Currently, there are three types of consent:
        //
        // pv: Can I send page views? Can I also capture active time?
        // ad: Can I show ads?
        // segment: can I run the segment gathering operation?
        ...
        setConsent: function(flags, options) {
            try {
                ...
                var flagsAll = this._applyObject({}, cX.allUserConsents() || {});
                ...
                // Refill the call queue with the previously denied requests.
                if (flags && rerunCallQueue && cX.Object.some(flags, function(value) { return value === true; })) {
                    var currCall = null;
        ...
        hasConsent: function(type) {
            // v2 consents must be explicitly turned on to take effect to ensure backwards compatibility.
            if (this.m_consentVersion !== 2 && this._isV2Consent(type)) return true;

            var flags = cX.allUserConsents() || {};
            return flags[type] === 'true';
        },

        _isV2Consent: function(type) { return this.consentClassesV2.find(function(v2consent) { return v2consent === type }) },

        _consentActiveAndAvailableFor: function(type) { return !this._doesNotHaveConsent(type) },

        _doesNotHaveConsent: function(type) { return this.m_usesConsent && !this.hasConsent(type); },

        _cmpFunc: function(funcName, value, callback) {
            if (window.__cmp) {
                window.__cmp(funcName, value, callback);
            } else {
                setTimeout(this.createDelegate(this, function() { this._cmpFunc(funcName, value, callback); }), 10);
            }
        },
        ...
        setupIabCmp: function(opts) {
            ...
            this._cmpFunc('getVendorConsents', [412], this.createDelegate(this, function(result) {
                try {
                    ...
                    if (!hasFullConsent) {
                        this.setConsent({ id: false, pv: false, ad: false, segment: false, recs: false });
                        this.clearIds();
                        return;
                    }
                    ...
                    if (this._doesNotHaveConsent('pv')) {
                        this.setConsent({ pv: false, ad: false, segment: false, recs: false });
                        this.clearIds();
                    }
                } catch(e) {}
            }));
        ...
        addCustomerScript: function(csId, asAsync) {
            ...
            // ensure that we load the domain script, but only once
            if (!document.getElementById('csd_domainScript')) {
                this.loadScript(domainSrc,
                                shouldAsync,
                                'UTF-8',
                                'csd_domainScript');
            }
            ...
            if (!document.getElementById('cs_' + csId)) {
                this.loadScript(this.csdUrls.customerScriptUrl + csId + '.js',
                                shouldAsync, 'UTF-8', 'cs_' + csId);
            }
        },
        ...
        _createCommsIFrame: function(args) {
            ...
            if (args.id) {
                iframeEl.id = args.id;
                iframeEl.name = args.id; // Set to fix https://jira.cxense.com/browse/CXSUP-4191
            }
            iframeEl.width = 0;
            iframeEl.height = 0;
        ...
        addExternalId: function(params) {
            if (this._doesNotHaveConsent('pv')) return;
        ...
        setCustomParameters: function(parameters, prefix) {
            ...
            for (key in parameters) {
                if (parameters.hasOwnProperty(key)) {
                    ...
                    // Use a maximum length of the provided size the key and the provided size for the value.
        ...
        isSafari: function() {
            try { return !!(navigator.userAgent.match(/Safari/) && !navigator.userAgent.match(/Android|Chrome|bot|link/)) } catch (e) { return false; }
        },
        ...
        isEdge: function() {
            try { return navigator.userAgent.match(/Edg\/8/); } catch (e) { return false; }
        },
        ...
        Object: {
            ...
            keys: function(object) {
                var keys = [];
                this.forEach(object, function(value, key) { keys.push(key); });
                return keys;
            }
        },
        ...
        sendPageViewEvent: function(args, callback) {
            ...
            if (this.m_previousPageViewReport !== pageView) {
                ...
                if (document.images) {
                    ...
                    var useP1 = false;
                    try {
                        if (!(this.isSafari() || this.isFirefox()) && this.hasLocalStorage()) {
                    ...
                    try {
                        if ((this.isSafari() || this.isFirefox() || this.isEdge()) && this.hasSessionStorage()) {
                    ...
                    try { repSrc += '&glb=' + encodeURIComponent(globalUserId || ''); } catch (e) { }
                    ...
                    if (this._consentActiveAndAvailableFor('geo') && (this.m_plat || this.m_plon)) {
                        try { repSrc += '&plat=' + encodeURIComponent(this.m_plat || ''); } catch (e) { }
                        try { repSrc += '&plon=' + encodeURIComponent(this.m_plon || ''); } catch (e) { }
                    }
                    ...
                    try { var ogsFieldModifiedTime = this._ogsFieldModifiedTime();
                          if (ogsFieldModifiedTime) { repSrc += '&amo=' + encodeURIComponent(ogsFieldModifiedTime); } } catch (e) { }
                    ...
                    try {
                        ...
                        if (!hasFlash) {
                            try {
                                var axo = new window.ActiveXObject('ShockwaveFlash.ShockwaveFlash');
                    ...
                    if (lookupLocalCst) {
                        ...
                        if (this.isSafari()) {
                            var tsrUrl = this.tsridUrl +
                                '&usi' + encodeURIComponent(this.getUserId());
                            this.jsonpRequest(tsrUrl, function(data) {
                                if (data && typeof data.cst === 'string' &&
                                    data.cst.length === 40 && data.cst.match(/^[a-z0-9_+]+$/i)) {
                    ...
                    if (typeof callback === 'function') {
                        repEl.onload = function() { callback({ url: repSrc, context: allArgs, isAnonymous: false }); };
                    }
                }
            }
        },
        ...
        onP1: function(scriptToken) {
            this._createExpireTokenRequest(''); // deletes the expiretoken cookie.
        ...
        getUserId: function(createIfMissing) {
            ...
            if (createIfMissing !== false) {
                ...
                try { this._setLocalStorage('_cX_P', persistentUserId, 365); } catch (e) { }
            }
        ...
        _localCxenseUserId: function(newId) {
            ...
            try {
                if (newId !== undefined) {
                    try { this.deleteCookie('cX_G', '/'); } catch (e) { }
                    try { this.deleteCookie('cX_G', '/', location.hostname); } catch (e) { }
                    if (!this.isSafari()) {
                        try { this.setCookie('cX_G', newId, 365, '/', this.getTopLevelDomain()); } catch (e) { }
                    }
                    try { this._setLocalStorage('_cX_G', newId, 365); } catch (e) { }
                }

                var persistentUserIdCookie = this.getCookie('cX_G');
                var persistentUserIdLocalStorage = null;
                try { if (this.hasLocalStorage()) { persistentUserIdLocalStorage = localStorage.getItem('_cX_G'); } } catch (e) { }
        ...
        _cxenseUserIdRequest: function(ckp, lst, cst) {
            try {
                var identities = [];
                identities.push({ type: 'ckp', id: ckp });
                if (lst) { identities.push({ type: 'lst', id: lst }); }
                if (cst) { identities.push({ type: 'cst', id: cst }); }
                var url = this.cxenseUserIdUrl + '?' +
                        'json=' + encodeURIComponent(cX.JSON.stringify({
                ...
                // Must use public `createDelegate`; Safari complains about `this`
        ...
        clearIds: function() {
            if (this.hasLocalStorage()) {
                var localStorageTokens = ['cX_lastP1Time',
                                          'cX_lst',
                                          '_cX_S',
                                          '_cX_P',
                                          '_cX_G',
            ...
            cX.Array.forEach(cookieTokens, this.createDelegate(this, function (token) {
                if (cX.getCookie(token)) this._deleteCookie_fix(token, '/', this.getTopLevelDomain());
            }));

            var sessionStorageTokens = ['_cX_S',
                                        '_cX_SA',
                                        'cX_lastLocalCstTime',
                                        'cX_lastLocalCst'];
        ...
        onClearIds: function() {
            if (this.hasLocalStorage()) {
                ...
                if (localStorage.getItem('cX_lst')) localStorage.removeItem('cX_lst');
            }
        },
        ...
        sendEvent: function(type, customParameters, providedArgs, segmentIds) {
            ...
            this.jsonpRequest(url, function(result) {
                if (args.callback) {
                    args.callback(result, { url: url, request: requestObject });
                }
        ...
        getUserSegmentIds: function(args, requestObject) {
            ...
            var readQueryCache = this.createDelegate(this, function() {
                ...
                // First try reading from local storage
                ...
                try {
                    // If local storage is unavailable or there is no info yet then try reading the new cookie format
                    // (<persistedQueryId>_<time>_<ids>/<persistedQueryId>:<time>:<ids>|.. etc)
                    ...
                    if (newFormatQueryCache) {
                        cX.Array.forEach(newFormatQueryCache.split('/'), function(cachedQuery) {
                            ...
                            if (cacheTokens && cacheTokens.length === 3) {
                                ...
                                cachedQueries[persistedQueryId] = { requestTime: requestTime, segmentIds: segmentIds };
                            }
                        });
                    }
                } catch (e) { }
            ...
            var writeQueryCache = this.createDelegate(this, function(cachedQueries) {
                ...
                for (var persistedQueryId in cachedQueries) {
                    if (cachedQueries.hasOwnProperty(persistedQueryId)) {
                        ...
                        cachedQueriesList.push({ persistedQueryId: persistedQueryId, requestTime: cachedQuery.requestTime, segmentIds: cachedQuery.segmentIds });
                    }
                }
                cachedQueriesList.sort(function(a, b) { return b.requestTime - a.requestTime; });
                var cookieParts = [];
                ...
                try {
                    // First try writing into local storage
                    this._setLocalStorage('_cX_segmentInfo', cookieParts.join('/'), 365);
                } catch(e) {
                    // if by some reason local storage fails to save info then set a cookie
                    // most browsers have a cookie limit of 4kb (4096 characters) per domain
                    // so, restricting segments info and leaving some free space
                    while (cookieParts.join('/').length > 1000) {
                        // remove last cached query as it doesn't fit into cookie
            ...
            if (!cachedQuery) {
                 cachedQuery = cachedQueries[args.persistedQueryId] = { requestTime: 0, segmentIds: args.defaultValue || [] };
            }
            ...
            if (isNaN(cachedSegmentTime) || (cachedSegmentTime < now - maxAge) ||
                cachedSegmentTime > now || (cX.isArray(segmentIds) && segmentIds.length === 0)) {
                ...
                this.jsonpRequest(url, function(data) {
                    ...
                    if (cX.isArray(newSegmentIds) && newSegmentIds.length > 0) {
                        ...
                        cachedQueries = readQueryCache(); // Could have changed while the request was sent
        ...
        getLocalStats: function() {
            ...
            var retVal = {};
            var map = {
                'psst': { name: 'prevSessionStartTime', func: function(v) { return v ? new Date(parseInt(v)) : undefined; } },
                'csst': { name: 'currSessionStartTime', func: function(v) { return v ? new Date(parseInt(v)) : undefined; } }
            };
        ...
        requireSecureCookies: function() { this.m_usesSecureCookies = true; },
        ...
        _assignCookieValue: function(name, value, expireDays, path, domain, secure) {
            var cookieToSet =
                ...
                (secure || this.m_usesSecureCookies ? ';secure' : '') +
                (this._canSetSameSite() ? ';SameSite=None': '');
        ...
        _canSetSameSite: function() {
            ...
            var isCorrectSafari = this.isSafari() &&
                (navigator.userAgent.indexOf('CPU iPhone OS 12') === -1 ||
                    navigator.userAgent.indexOf('iPad; CPU OS 12') === -1 ||
                    (navigator.userAgent.indexOf('Macintosh; Intel Mac OS X 10_14') === -1 && navigator.userAgent.indexOf('Version/') === -1));
        ...
        setCookie: function(name, value, expireDays, path, domain, secure) {
            ...
            // Limit to 1K assuming string values are 1 byte )(
            if (this._byteCount(value) > 1024) { return; }
        ...
        _deleteCookie_fix: function(name, path, domain, secure) {
            ...
            document.cookie = encodeURIComponent(name) +
                  ...
                  (domain ? "; domain=" + domain : "") +
                  (path ? "; path=" + path : "") +
                  ((secure || this.m_usesSecureCookies) ? 'SameSite=None;secure': '');
        },
        ...
        getNodeValue: function(node) {
            // Cannot use feature detection here, as .value exists, but has corrupt value on older IEs. See http://www.quirksmode.org/dom/core/
        ...
        insertMultipleAdSpaces: function(argsArray, optionalCommonArgs) {
            if (this._doesNotHaveConsent('ad')) return;
            var adSpaceIds = [];
        ...
        _insertWidget: function(args, requestObject) {
            ...
            // Set the consent values for the request.
            if (this.m_usesConsent) {
                ...
                if (!allArgs.adSpaceId && !allArgs.adSpaceIds && (allArgs.widgetId || requestObject.widgetId)) {
                    ...
                    if (!this.isConsentRequired() || (this.isConsentRequired() && this.hasConsent('pv'))) {
                        if (allArgs.usi || persistentUserId) {
                            ...
                            // Do not send anything if there is no user defined at all.
        ...
        getScrollPos: function() {
            ...
            if (this.m_activityState.currScrollTop || this.m_activityState.currScrollLeft){
                //'overflow-x: hidden'/'height: 100%' bug fix
        ...
        _onPollActivity: function() {
            if (!this._shouldPollActivity()) { return; }
        ...
        cint: function(prefix, campaignId) {
            if (this.isFirefox() || this.isSafari()) return;

            if (prefix !== "219" && prefix !== "220" && prefix !== "221" &&
                prefix !== "222" && prefix !== "223" && prefix !== "0037") {
                return;
            }

            if (this._doesNotHaveConsent('segment') ||
                cX.getCookie('cX_cint_set') === '1' ||
                (this.hasLocalStorage() && localStorage.getItem('cX_cint_set') === '1') ||
                cX.getCookie('cbcdfp') !== null) {
                return;
            }
            ...
            cX.Array.forEach(arguments, function(provider) {
                ...
                try {
                    switch (this._getNumberOfSyncWithExternalParty(provider)) {
                        case 1: this._setTimeoutInCookie(1); break;
                        case 2: this._setTimeoutInCookie(604800); break;
                        default: return;
                    }
                } catch (e) {}
                ...
                switch (provider) {
                    case 'pubmatic':
                        imageFunction = function(cxenseUserId, thisObj) {thisObj._createCommsIFrame({ src: 'https://ads.pubmatic.com/AdServer/js/user_sync.html?p=' + customerId + '&predirect=https%3A%2F%2Fcsyn.cxense.com%2Fpcs.gif%3Fcxsite%3D' + siteIdParam + '%26userId%3D'});};
                        break;
                    case 'bla': imageFunction = function(cxenseUserId, thisObj) {new Image().src = 'https://aid.send.microad.jp/asr?v=1&code=dY-5ZLLSddc&format=pixel'}; break;
                    case 'smart': imageFunction = function(cxenseUserId) {new Image().src = 'https://sync.smartadserver.com/getuid?url=https%3A%2F%2Fcsyn.cxense.com%2Fsmt.png%3Fcxsite%3D' + siteIdParam + '%26userId%3D%5Bsas_uid%5D%26cxckp%3D' + cxenseUserId;}; break;
                    case 'simpli': imageFunction = function(cxenseUserId) {new Image().src = 'https://um.simpli.fi/cxense';}; break;
                    case 'xl8': imageFunction = function(cxenseUserId) {new Image().src = 'https://loadm.exelator.com/load/?p=204&g=820&j=0';}; break;
                    case 'neu': imageFunction = function(cxenseUserId) {new Image().src = 'https://aa.agkn.com/adscores/g.pixel?sid=9212271498';}; break;
                    case 'p161': imageFunction = function(cxenseUserId) {new Image().src = 'https://ads.creative-serving.com/cm?redir=https%3A%2F%2Fcsyn.cxense.com%2Fpfm.png%3Fcxsite%3D' + siteIdParam + '%26uuid%3D%24%7BUUID%7D%26cxckp%3D' + cxenseUserId;}; break;
                    case 'liveramp': imageFunction = function(cxenseUserId) {new Image().src = 'https://idsync.rlcdn.com/421746.gif?partner_uid=' + cxenseUserId;}; break;
                    case 'liveramp2': imageFunction = function(cxenseUserId) {new Image().src = 'https://csyn-r.cxense.com/?cxsite=' + siteIdParam + '&partnerId=lr2&redir=https%3A%2F%2Fidsync.rlcdn.com%2F421746.gif%3Fpartner_uid%3D%24%7BUUID%7D&cxtag';}; break;
                    case 'adform': imageFunction = function(cxenseUserId) {new Image().src = 'https://csyn-r.cxense.com/?cxsite=' + siteIdParam + '&partnerId=csr&redir=https%3A%2F%2Fdmp.adform.net%2Fserving%2Fcookie%2Fmatch%2F%3Fparty%3D1077%26cid%3D%24%7BUUID%7D%26cxckp%3D' + cxenseUserId;}; break;
                    case 'ddp': imageFunction = function(cxenseUserId) {new Image().src = 'https://cm.g.doubleclick.net/pixel?google_nid=cxense_ddp&google_cm&cxckp=' + cxenseUserId + '&cxsite=' + siteIdParam;}; break;
                    case 'kccs': imageFunction = function(cxenseUserId) {new Image().src = 'https://kcxcs.deqwas.net/DmpSyncService/Sync.aspx';}; break;
                    case 'scaleout': imageFunction = function(cxenseUserId) {new Image().src = 'https://tg.socdm.com/aux/idsync?proto=cxense';}; break;
                    case 'aone': imageFunction = function(cxenseUserId) {new Image().src = 'https://aw.dw.impact-ad.jp/c/u/?oid=' + customerId + '&rdr=https://csyn.cxense.com/aon.png?customerId=' + customerId + '%26uid%3D%7BAONEID%7D%26cxckp%3D' + cxenseUserId;}; break;
                    case 'frk': imageFunction = function(cxenseUserId) {new Image().src = 'https://sync.dmp.fout.jp/serve/?id=13336&mt=213';}; break;
                    case 'appnexus': imageFunction = function(cxenseUserId) {new Image().src = 'https://secure.adnxs.com/getuid?https%3A%2F%2Fcsyn.cxense.com%2Fapn.png%3Fcxsite%3D' + siteIdParam + '%26uid%3D%24UID%26cxckp%3D' + cxenseUserId;}; break;
                    case 'aam': imageFunction = function(cxenseUserId) {new Image().src = 'https://dpm.demdex.net/ibs:redir=https%3a%2f%2fcsyn%2ecxense%2ecom%2faam%2epng%3fcxsite%3D' + siteIdParam + '%26userId%3d%24%7bDD_UUID%7d%26cxckp%3D' + cxenseUserId;}; break;
                    case 'xro': imageFunction = function(cxenseUserId) {new Image().src = 'https://dex.advg.jp/dx/p/sync?_aid=111&_page=2257';}; break;
                    case 'pdx': imageFunction = function(cxenseUserId) {new Image().src = 'https://tags.bluekai.com/site/48528?redir=https%3A%2F%2Fcsyn.cxense.com%2Fpdx.png%3Fcxsite%3D' + siteIdParam + '%26userId%3D%24_BK_UUID%26cxckp%3D' + cxenseUserId;}; break;
                    case 'frw': imageFunction = function(cxenseUserId) {new Image().src = 'https://ads.stickyadstv.com/data-registering?dataProviderId=1233&redirectId=1617';}; break;
                    case 'adition': imageFunction = function(cxenseUserId) {new Image().src = 'https://dsp.adfarm1.adition.com/cookie/?redirect=https%3A%2F%2Fcsyn.cxense.com%2Fsia.png%3Fcxsite%3D' + siteIdParam + '%26userId%3D%25%25COOKIE%25%25%26cxckp%3D' + cxenseUserId;}; break;
                    case 'imdigital': imageFunction = function(cxenseUserId) {new Image().src = 'https://ad.360yield.com/ux?publisher_id=' + customerId + '&publisher_dmp_id=5&r=https%3A%2F%2Fcsyn.cxense.com%2Fcoo.png%3Fcxsite%3D' + siteIdParam + '%26userId%3D%7bPUB_USER_ID%7d%26cxckp%3D' + cxenseUserId;}; break;
                    case 'admatrix': imageFunction = function(cxenseUserId) {new Image().src = 'https://csyn-r.cxense.com/?cxsite=' + siteIdParam + '&partnerId=adm&redir=https%3A%2F%2Frelay-dsp.t-ad-m.asia%2Fdmp%2Fsync%2Fbizmatrix%2Fasync%3Fpid%3D' + customerId + '%26uid%3D' + cxenseUserId;}; break;
                    case 'rub': imageFunction = function(cxenseUserId) {new Image().src = 'https://csyn-r.cxense.com/?cxsite=' + siteIdParam + '&partnerId=rub&redir=https' + encodeURIComponent('://token.rubiconproject.com/token?pid=34138&puid=${UUID}' + (cX.getCookie('euconsent') ? ('&gdpr=1&gdpr_consent=' + cX.getCookie('euconsent')) : '&gdpr=0&gdpr_consent=') + '&cxckp=' + cxenseUserId);}; break;
                    case 'spx': imageFunction = function(cxenseUserId) {new Image().src = 'https://sync.search.spotxchange.com/audience_sync/19?redir=https' + encodeURIComponent('://csyn.cxense.com/spx.png?cxsite=' + siteIdParam + '&uuid=$SPOTX_AUDIENCE_ID&cxckp=' + cxenseUserId);}; break;
                    case 'lgd': imageFunction = function(cxenseUserId) {new Image().src = 'https://cr-p10350.ladsp.jp/pid/10350';}; break;
                    case 'imi': imageFunction = function(cxenseUserId) {new Image().src = 'https://sync.im-apps.net/imid/redirect?cid=' + customerId + '&tid=' + tmpProvider.testId + params;}; break;
                    case 'ttd': imageFunction = function(cxenseUserId) {new Image().src = 'https://match.adsrvr.org/track/cmf/generic?ttd_pid=' + customerId + '&ttd_tpi=1';}; break;
                    case 'test': imageFunction = function(cxenseUserId) {new Image().src = 'https://csyn-r-stage.cxense.com/?partnerId=test&redir=https%3A%2F%2Fcsyn-stage.cxense.com%2Ftest.gif%3FuserId%3D%24%7BUUID%7D';}; break;
                }
    ...
    // temp: remove segmentInfo cookie if we can assume it is already stored in localstorage
    ...
    function cx_callQueueExecute() {
        try {
            ...
            while (currCall = cX.callQueue.shift()) {
                if (cX.library._doesNotHaveConsent(cX.library.denyWithoutConsent[currCall[0]]) &&
                    cX.Array.some(denyMethods, function(meth) { return currCall[0] === meth })) {
    ...
    function cx_pollActivity() {
        if (!cX.library._shouldPollActivity()) { return; }
